...
# AI modules temporarily disabled - using mock implementation
# from ..ai.database import AnalysisResult, DatabaseManager, UserFeedback
...
# from ..ai.predictor import AIAssistant, TransitPredictor
# from ..ai.trainer import ModelTrainer, TransitDataset
from models.search import LightCurveData, SearchResult, TransitCandidate
from services.bls import BLSService
...
class AdvancedAIService:
    ...
    async def intelligent_analysis(self, lightcurve_data: Dict) -> AIAnalysisResult:
        ...
        for model_name, model in self.models.items():
            try:
                pred_result = await self._predict_with_model(
...
class AIEnhancedBLSService:
    ...
    def __init__(
        ...
        self.predictor = TransitPredictor(
            model=self.model, embedding_manager=self.embedding_manager, device=device
        )

        # Тренер для онлайн обучения
        self.trainer = ModelTrainer(
            model=self.model, device=device, experiment_name="ai_enhanced_bls"
        )
    ...
    async def get_ai_explanation(
        self, target_name: str, mode: str = "detailed"
    ) -> Dict[str, str]:
        """
        ...
        if mode == "beginner":
            ...
            explanations["statistical_significance"] = (
                self._assess_statistical_significance(cached_prediction)
            )
    ...
    async def retrain_model(
        self, dataset_name: str = "user_feedback", epochs: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        feedback_data = await self.db_manager.get_feedback_for_active_learning(
            limit=1000
        )
        ...
        training_result = await self.trainer.train_with_feedback(
            dataset=dataset, epochs=epochs, learning_rate=1e-4, batch_size=32
        )
    ...
    async def get_similar_targets(
        self, target_name: str, top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self.db_manager and target_embedding is not None:
            ...
            all_analyses = await self.db_manager.get_analysis_history_with_embeddings(
                limit=1000
            )
            ...
            for analysis in all_analyses:
                if (
                    analysis["target_name"] != target_name
                    and analysis.get("embedding") is not None
                ):
    ...
    async def _validate_candidates_with_ai(
        self, candidates: List[TransitCandidate], flux: np.ndarray, ai_prediction
        ...
        for candidate in candidates:
            ...
            if ai_prediction.is_transit and ai_prediction.confidence > 0.6:
                ...
                candidate.planet_probability = min(
                    candidate.planet_probability * 1.2, 0.99  # Увеличиваем вероятность
                )
                ...
                if (
                    candidate.planet_probability > 0.1
                ):  # Оставляем только если вероятность не слишком мала
                    validated_candidates.append(candidate)
            else:
    ...
    def _enhance_search_result(
        self, bls_result: SearchResult, ai_predictions: List
    ) -> SearchResult:
    ...
    async def _check_and_trigger_online_learning(self):
        ...
        recent_feedback = await self.db_manager.get_feedback_for_active_learning(
            limit=50
        )
    ...
    async def _background_retrain(self, feedback_data: List):
        ...
        try:
            logger.info(
                f"Starting background retraining with {len(feedback_data)} samples"
            )
    ...
    async def _prepare_training_data_from_feedback(self, feedback_data: List) -> Dict:
        ...
        for feedback in feedback_data:
            ...
            analysis = await self.db_manager.get_analysis_by_target(
        ...
        return {"lightcurves": np.array(lightcurves), "labels": np.array(labels)}
