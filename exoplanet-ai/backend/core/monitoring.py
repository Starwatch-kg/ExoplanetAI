...
class MetricsCollector:
    ...
    def get_endpoint_performance(self, minutes: int = 60) -> Dict[str, Dict[str, float]]:
...
class PerformanceProfiler:
    ...
    def _analyze_api_performance(self) -> Dict[str, Any]:
        ...
        performance_grade = 'A' if avg_response_time < 500 and error_rate < 0.01 else \
                          'B' if avg_response_time < 1000 and error_rate < 0.05 else \
                          'C' if avg_response_time < 2000 and error_rate < 0.1 else 'D'
        ...
        for endpoint, stats in endpoint_perf.items():
            if stats['avg_response_time'] > 2000:
                bottlenecks.append({
                    ...
                    'severity': 'high' if stats['avg_response_time'] > 5000 else 'medium'
                })
