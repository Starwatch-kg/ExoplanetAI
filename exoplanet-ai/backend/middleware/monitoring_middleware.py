...
from core.monitoring import get_metrics_collector, APIMetrics
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, self.requests_per_minute - minute_count - 1))
        response.headers["X-RateLimit-Reset"] = str(int((int(current_time // 60) + 1) * 60))
