...
class DatabaseManager:
    ...
    async def _populate_initial_data(self):
        ...
        logger.info(f"✅ Populated database with {len(initial_exoplanets)} initial exoplanets")
    ...
    async def insert_exoplanet(self, exoplanet: ExoplanetRecord) -> int:
        ...
        async with self._lock:
            ...
            """, (
                exoplanet.name, exoplanet.host_star, exoplanet.discovery_method,
                exoplanet.discovery_year, exoplanet.orbital_period_days,
                exoplanet.radius_earth_radii, exoplanet.mass_earth_masses,
                exoplanet.equilibrium_temperature_k, exoplanet.distance_parsecs,
                exoplanet.confidence, exoplanet.status, exoplanet.habitable_zone
            ))
            conn.commit()
    ...
    async def insert_search_result(self, result: SearchResult) -> int:
        ...
        async with self._lock:
            ...
            """, (
                result.target_name, result.catalog, result.mission, result.method,
                result.exoplanet_detected, result.detection_confidence,
                result.processing_time_ms, result.result_data
            ))
            conn.commit()
            return cursor.lastrowid
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            cursor = conn.execute("SELECT COUNT(*) FROM exoplanets WHERE status = 'confirmed'")
            stats['confirmed_exoplanets'] = cursor.fetchone()[0]
            
            cursor = conn.execute("SELECT COUNT(*) FROM exoplanets WHERE habitable_zone = 1")
