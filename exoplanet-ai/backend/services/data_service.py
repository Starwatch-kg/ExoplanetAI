...
class DataService:
    ...
    async def get_lightcurve(self, target_name: str, mission: str) -> Optional[LightcurveData]:
    ...
    async def _query_tic_catalog(self, tic_id: str) -> StarInfo:
        ...
        def _sync_query():
            ...
            return StarInfo(
                ...
                stellar_type=str(row.get('SpType', 'Unknown')) if row.get('SpType') else None
            )
    ...
    async def _download_tess_lightcurve(self, target_name: str) -> Optional[LightcurveData]:
        ...
        def _sync_download():
            ...
            if len(search_result) == 0:
                ...
                logger.info(f"No direct TESS data found for {target_name}, trying alternative searches...")
                ...
                if len(search_result) == 0:
                    # Try with TIC prefix
                    search_result = lk.search_lightcurve(f"TIC {clean_name}", mission='TESS')
                
                if len(search_result) == 0:
                    raise ValueError(f"No TESS data found for {target_name}")
            
            logger.info(f"Found {len(search_result)} TESS observations for {target_name}")
            
            # Download the first available lightcurve
            lc = search_result[0].download()
            
            if lc is None:
                raise ValueError(f"Failed to download TESS lightcurve for {target_name}")
            ...
            logger.info(f"Successfully downloaded TESS lightcurve: {len(time_array)} points, noise: {noise_ppm:.1f} ppm")
    ...
    async def _download_kepler_lightcurve(self, target_name: str) -> Optional[LightcurveData]:
        ...
        def _sync_download():
            ...
            if lc is None:
                raise ValueError(f"Failed to download Kepler lightcurve for {target_name}")
    ...
    async def _download_k2_lightcurve(self, target_name: str) -> Optional[LightcurveData]:
