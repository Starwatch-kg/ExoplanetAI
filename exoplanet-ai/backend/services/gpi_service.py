...
from ml.gpi_ai_model import GPIAIModel, ML_AVAILABLE
...
class AsyncCache:
    ...
    def _generate_key(self, target_data: Dict, custom_params: Optional[Dict] = None) -> str:
        """Генерация ключа кэша"""
        key_data = {
            'target_name': target_data.get('target_name', ''),
            'data_length': len(target_data.get('time', [])),
            'data_hash': hashlib.md5(str(target_data.get('time', [])[:10]).encode()).hexdigest()[:8],
            'params': custom_params or {}
        }
        return hashlib.md5(str(sorted(key_data.items())).encode()).hexdigest()
    
    async def get(self, target_data: Dict, custom_params: Optional[Dict] = None) -> Optional[Dict]:
    ...
    async def set(self, target_data: Dict, result: Dict, custom_params: Optional[Dict] = None):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'oldest_entry_age': time.time() - min(self._access_times.values()) if self._access_times else 0
            }
...
class GPIService:
    ...
    def __init__(self, 
        ...
        if ML_AVAILABLE:
            try:
                ...
                model_path = Path(__file__).parent.parent / "models" / "gpi_ai_model.pkl"
        ...
        logger.info(f"Advanced GPI Service initialized: cache_size={cache_size}, "
                   f"thread_pool={thread_pool_size}, process_pool={process_pool_size}")
    
    @async_retry(max_retries=2, delay=0.5)
    async def analyze_target(self, 
                           target_data: Dict,
                           use_ai: bool = True,
        ...
        try:
            ...
            if use_cache:
                ...
                if cached_result:
                    ...
                    cached_result['service_info']['analysis_time'] = datetime.now().isoformat()
            ...
            validation_task = asyncio.create_task(
                self._validate_lightcurve_data_async(time_array, flux_array, flux_err_array)
            )
            ...
            if not validation_result['is_valid']:
                logger.warning(f"Data validation issues for {target_name}: {validation_result['issues']}")
            ...
            logger.info(f"Advanced GPI analysis completed for {target_name} in {processing_time:.1f}ms")
            return gpi_result
    ...
    async def _run_gpi_analysis_optimized(self, 
                                     time: np.ndarray, 
                                     flux: np.ndarray,
                                     flux_err: Optional[np.ndarray],
                                     target_name: str,
                                     custom_params: Optional[Dict]) -> Dict:
        ...
        if data_size > 10000:  # Large dataset - use process pool
            task = loop.run_in_executor(
                ...
                time, flux, flux_err, target_name, custom_params
            )
    ...
    async def _validate_lightcurve_data_async(self, 
                                            time: np.ndarray, 
                                            flux: np.ndarray, 
                                            flux_err: Optional[np.ndarray]) -> Dict:
        ...
        def _validate():
            ...
            if len(time) < 50:
                issues.append("Too few data points (minimum 50 required for reliable GPI)")
            ...
            if duration < 5:  # Less than 5 days
                issues.append(f"Observation duration too short for GPI: {duration:.1f} days")
            ...
            return {
                ...
                'cadence_minutes': float(median_cadence * 24 * 60) if len(time) > 1 else 0.0,
    ...
    def _validate_lightcurve_data(self, 
                                time: np.ndarray, 
                                flux: np.ndarray, 
                                flux_err: Optional[np.ndarray]) -> Dict:
        ...
        try:
            ...
            if self.ai_model and self.ai_model.is_trained:
                try:
                    ...
                    deterministic_noise = 0.001 * np.sin(time_array * 0.1) * np.cos(time_array * 0.05)
                    test_data = {
                        'time': time_array,
