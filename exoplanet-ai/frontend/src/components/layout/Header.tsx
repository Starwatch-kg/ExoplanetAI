...
const Header: React.FC<HeaderProps> = ({ healthStatus }) => {
  ...
  useEffect(() => {
    const fetchSystemStats = async () => {
      try {
        const [cppResponse, dbResponse] = await Promise.all([
        ...
        setSystemStats({
          activeUsers: dbData.active_sessions || 1,
          totalSearches: dbData.total_searches || 0,
          cppStatus: cppData.overall_status || 'unknown',
    ...
    fetchSystemStats()
    const interval = setInterval(fetchSystemStats, 30000) // Update every 30s
            ...
            <div className="mt-4 space-y-2">
              {healthStatus && (
                <div className={`flex items-center gap-2 px-4 py-3 rounded-lg text-sm ${
                  ...
                  <span>
                    API: {healthStatus.status} • v{healthStatus.version}
                  </span>
                </div>
              )}
