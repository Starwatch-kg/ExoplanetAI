...
const NASADataBrowser: React.FC = () => {
  ...
  const [nasaStats, setNasaStats] = useState<{
    totalPlanets: number;
    totalHosts: number;
    lastUpdated: string;
    source: string;
  ...
  // Загрузка статистики NASA через наш API
  const loadNASAStats = async () => {
    try {
      const stats = await exoplanetApi.getNASAStats();
      setNasaStats(stats);
    } catch (error) {
      console.error('Ошибка загрузки статистики NASA:', error);
      // Fallback статистика
      setNasaStats({
        totalPlanets: 5635,
        totalHosts: 4143,
  ...
  useEffect(() => {
    // Загружаем статистику NASA при монтировании компонента
    loadNASAStats();
      ...
      {nasaStats && (
        <div className="mb-6 p-4 bg-slate-800/50 rounded-lg border border-slate-600">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-blue-400">{nasaStats.totalPlanets.toLocaleString()}</div>
              <div className="text-sm text-slate-400">Подтвержденных планет</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-green-400">{nasaStats.totalHosts.toLocaleString()}</div>
              <div className="text-sm text-slate-400">Звезд-хозяев</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-400">{exoplanets.length}</div>
              <div className="text-sm text-slate-400">Загружено сейчас</div>
            </div>
            <div>
              <div className="text-xs text-slate-500">{nasaStats.source}</div>
              <div className="text-xs text-slate-500">
                {new Date(nasaStats.lastUpdated).toLocaleString()}
              </div>
            </div>
          </div>
        </div>
