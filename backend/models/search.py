...
class LightCurveData(BaseModel):
    ...
    target_name: str = Field(
        ..., min_length=1, max_length=100, description="Название цели"
    )
    ...
    duration_days: float = Field(
        0.0, ge=0.0, description="Продолжительность наблюдений (дни)"
    )
...
class TargetInfo(BaseModel):
    ...
    catalog_id: str = Field(
        ..., min_length=1, max_length=50, description="ID в каталоге"
    )
    coordinates: Dict[str, float] = Field(
        ..., description="Координаты {'ra': float, 'dec': float}"
    )
    magnitude: float = Field(..., ge=0.0, le=30.0, description="Звездная величина")
    available_data: List[str] = Field(
        [], description="Доступные данные ['TESS', 'Kepler', ...]"
    )
...
class TransitCandidate(BaseModel):
    ...
    planet_radius: Optional[float] = Field(
        None, gt=0, description="Радиус планеты (R_earth)"
    )
    semi_major_axis: Optional[float] = Field(
        None, gt=0, description="Большая полуось (AU)"
    )
    equilibrium_temp: Optional[float] = Field(
        None, gt=0, description="Равновесная температура (K)"
    )
    false_alarm_probability: float = Field(
        0.5, ge=0, le=1, description="Вероятность ложного срабатывания"
    )
    planet_probability: float = Field(
        0.5, ge=0, le=1, description="Вероятность наличия планеты"
    )
...
class BLSResult(BaseModel):
    ...
    best_duration: float = Field(
        ..., gt=0, description="Лучшая длительность транзита (дни)"
    )
    ...
    significance: float = Field(
        ..., ge=0, le=1, description="Статистическая значимость"
    )
...
class SearchRequest(BaseModel):
    ...
    mission: str = Field("TESS", pattern="^(TESS|Kepler|K2)$", description="Миссия")
    period_min: float = Field(
        0.5, ge=0.1, le=100.0, description="Минимальный период (дни)"
    )
    period_max: float = Field(
        20.0, ge=0.1, le=100.0, description="Максимальный период (дни)"
    )
    duration_min: float = Field(
        0.05, ge=0.01, le=1.0, description="Минимальная длительность (дни)"
    )
    duration_max: float = Field(
        0.3, ge=0.01, le=1.0, description="Максимальная длительность (дни)"
    )
