...
class GPIFeatureExtractor:
    ...
    def _extract_statistical_features(
        ...
        if SCIPY_AVAILABLE:
            features.extend([stats.skew(flux), stats.kurtosis(flux)])
        ...
        if len(power_pos) > 0:
            ...
            features.extend(
                [
                    ...
                    (
                        1.0 / freqs_pos[max_power_idx]
                        if freqs_pos[max_power_idx] > 0
                        else 0
                    ),
                ]
            )
    ...
    def _extract_gpi_features(
        ...
        if SCIPY_AVAILABLE:
            ...
            names.extend(
                ["phase_std", "phase_variation", "max_phase", "phase_coherence"]
            )

        # Периодичность (поиск повторяющихся паттернов)
        autocorr_features, autocorr_names = self._calculate_autocorrelation_features(
            flux
        )
    ...
    def _calculate_autocorrelation_features(
        self, flux: np.ndarray
    ) -> Tuple[List[float], List[str]]:
...
class GPIAIModel:
    ...
    def prepare_training_data(
        ...
        for sample in training_samples:
            try:
                ...
                logger.warning(
                    f"Ошибка обработки образца {sample.get('target_name', 'unknown')}: {e}"
                )
                continue
        ...
        return {
            **prediction_result,
            ...
            "explanation": self._generate_explanation(
                sorted_contributions, prediction_result
            ),
        }
    ...
    def _generate_explanation(self, contributions: Dict, prediction: Dict) -> str:
        ...
        if has_planet:
            ...
            explanation = (
                f"Модель не обнаружила планету (уверенность {confidence:.1%}). "
            )
    ...
    def get_model_performance(self) -> Dict:
        ...
        performance = {
            **self.model_info,
            "model_complexity": {
                "n_estimators": len(
                    self.pipeline.named_steps["classifier"].estimators_
                ),
    ...
    def predict_with_uncertainty(self, lightcurve_data: Dict) -> Dict:
        ...
        uncertainty_metrics = {
            ...
            "agreement_ratio": float(
                np.sum(np.array(planet_probs) > 0.5) / len(planet_probs)
            ),
            "entropy": float(
                -np.sum([p * np.log2(p + 1e-10) for p in planet_probs])
                / len(planet_probs)
            ),
        }
        ...
        return {
            **base_prediction,
            ...
            "confidence_level": (
                "high"
                if uncertainty_metrics["std_probability"] < 0.1
                else "medium" if uncertainty_metrics["std_probability"] < 0.2 else "low"
            ),
        }
