...
class GPIEngine:
    ...
    def test_system(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Self-test requires real astronomical data - skipping synthetic test"
            )
            return True
    ...
    def analyze_lightcurve(
        ...
        try:
            ...
            planetary_params = self._calculate_planetary_parameters(
            ...
            logger.info(
                f"GPI analysis completed for {target_name}: "
    ...
    def _extract_gravitational_phase(
    ...
    def _process_gpi_signal(
        self, time: np.ndarray, phase_data: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _detect_orbital_perturbations(
        self, time: np.ndarray, gpi_signal: np.ndarray
    ) -> Dict:
        """
        ...
        for method_name, frequencies, psd in results:
            ...
            valid_mask = (
                (periods >= self.params.min_period_days)
    ...
    def _calculate_planetary_parameters(self, perturbation_analysis: Dict) -> Dict:
        ...
        """
        if (
            perturbation_analysis.get("max_phase_shift", 0)
            < self.params.phase_sensitivity
        ):
            return {}
        ...
        a_cubed = (
            period_seconds**2 * self.params.gravitational_constant * stellar_mass_kg
        ) / (4 * np.pi**2)
    ...
    def _validate_gpi_detection(
        self, perturbation_analysis: Dict, planetary_params: Dict
    ) -> float:
        """
    ...
    def _compile_gpi_result(
        ...
        return {
            ...
            "gpi_analysis": {
                "phase_shift_amplitude_rad": float(
                    perturbation_analysis.get("max_phase_shift", 0)
                ),
                "orbital_period_days": float(
                    perturbation_analysis.get("best_period", 0)
                ),
                "gravitational_signature_strength": float(
                    perturbation_analysis.get("signature_strength", 0)
                ),
            ...
            "planetary_characterization": {
                ...
                "detection_metrics": {
                    ...
                    "gravitational_periodogram_peak": float(
                        perturbation_analysis.get("periodogram_peak", 0)
                    ),
                    "false_alarm_probability": float(
                        np.exp(-detection_confidence / 2.0)
                    ),
                },
            },
            "technical_details": {
                ...
                "parameters_used": {
                    ...
                    "period_range": [
                        self.params.min_period_days,
                        self.params.max_period_days,
                    ],
                },
            },
            "metadata": {
