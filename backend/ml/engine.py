...
class InferenceEngine:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if ML_AVAILABLE:
                ...
                logger.warning(
                    "ML libraries not available, using simple classifier only"
                )
    ...
    async def predict(
        self, time: np.ndarray, flux: np.ndarray, model_name: str = "auto"
    ) -> InferenceResult:
        """
        ...
        try:
            ...
            result = await loop.run_in_executor(
                self.executor, selected_model.predict, features
            )
