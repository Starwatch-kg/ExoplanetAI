...
=======
Unified Production Data Service
Объединенный продакшен сервис данных для астрономических вычислений
"""
...
try:
    ...
    class MockMetrics:
        def record_cache_operation(self, *args): pass
...
@dataclass
class LightcurveInfo:
    """Lightcurve information container"""
    time: np.ndarray
    flux: np.ndarray
    flux_err: np.ndarray
    mission: str
...
=======
    ...
    async def get_star_info(self, target_name: str, catalog: str, use_nasa_data: bool = True) -> Dict:
        ...
        # Проверяем кэш
        cache_key = self._get_cache_key("star", catalog, target_name)
...
<<<<<<< HEAD
=======
                self._set_cache(cache_key, nasa_data)
>>>>>>> 975c3a7 (Версия 1.5.1)
                return nasa_data
...
=======
        ...
        self._set_cache(cache_key, star_data)
        return star_data
>>>>>>> 975c3a7 (Версия 1.5.1)
...
=======
        """Unified BLS analysis using optimized service"""
        
        # Use unified BLS service if available
        if bls_service:
            try:
                ...
                logger.warning(f"Unified BLS service failed: {e}, using fallback")
        
        # Fallback to simple BLS implementation
...
=======
production_data_service = UnifiedDataService()
>>>>>>> 975c3a7 (Версия 1.5.1)
