...
from services.bls_service import BLSService, BLSResult
...
class UltimateEnsembleSearchEngine:
    ...
    async def ultimate_search(
        ...
        try:
            ...
            time_clean, flux_clean, flux_err_clean = await self._ultimate_data_preprocessing(
                time, flux, flux_err
            )
            ...
            search_tasks.append(
                asyncio.create_task(
                    self._run_bls_search(time_clean, flux_clean, flux_err_clean, 
                                       period_min, period_max, target_name)
                )
            )
            ...
            search_tasks.append(
                asyncio.create_task(
                    self._run_wavelet_analysis(time_clean, flux_clean)
                )
            )
            ...
            search_tasks.append(
                asyncio.create_task(
                    self._run_fourier_analysis(time_clean, flux_clean, period_min, period_max)
                )
            )
            ...
            if self.ml_ensemble:
                search_tasks.append(
                    asyncio.create_task(
                        self._run_ml_ensemble_search(all_features, time_clean, flux_clean)
                    )
                )
            ...
            for i, result in enumerate(search_results):
                method_names = ['bls', 'gpi', 'tls', 'wavelet', 'fourier', 'ml_ensemble']
                method_name = method_names[i] if i < len(method_names) else f'method_{i}'
            ...
            await self._perform_physical_modeling(
                ensemble_result, stellar_params or {}
            )
            ...
            final_result = EnsembleSearchResult(
                ...
                false_positive_probability=ensemble_result['false_positive_probability'],
        ...
        if len(flux_clean) > 100:
            try:
                ...
                features = np.column_stack([
                    StandardScaler().fit_transform(time_clean.reshape(-1, 1)).flatten(),
                    StandardScaler().fit_transform(flux_clean.reshape(-1, 1)).flatten()
                ])
    ...
    async def _extract_all_features(self, time: np.ndarray, flux: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    def _extract_statistical_features(self, time: np.ndarray, flux: np.ndarray) -> np.ndarray:
        """Статистические признаки"""
        features = []
        
        # Базовая статистика
        features.extend([
            np.mean(flux), np.std(flux), np.var(flux), np.median(flux),
            np.min(flux), np.max(flux), np.ptp(flux),
            stats.skew(flux), stats.kurtosis(flux)
        ])
    ...
    def _extract_frequency_features(self, time: np.ndarray, flux: np.ndarray) -> np.ndarray:
        ...
        if len(power_pos) > 0:
            ...
            if total_power > 0:
                ...
                # Спектральная ширина
                spectral_spread = np.sqrt(np.sum(((freqs_pos - spectral_centroid)**2) * power_pos) / total_power)
                features.append(spectral_spread)
                
                # Спектральная асимметрия
                spectral_skewness = np.sum(((freqs_pos - spectral_centroid)**3) * power_pos) / (total_power * spectral_spread**3)
    ...
    def _extract_wavelet_features(self, time: np.ndarray, flux: np.ndarray) -> np.ndarray:
        """Вейвлет признаки"""
        features = []
    ...
    def _extract_chaos_features(self, time: np.ndarray, flux: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if len(flux) > m * tau:
                ...
                embedded = np.array([flux[i:i+m*tau:tau] for i in range(len(flux) - m*tau + 1)])
    ...
    def _extract_information_features(self, time: np.ndarray, flux: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            lz_complexity = len(substrings) / len(binary_string) if len(binary_string) > 0 else 0
    ...
    async def _run_bls_search(self, time: np.ndarray, flux: np.ndarray, flux_err: Optional[np.ndarray],
                            period_min: float, period_max: float, target_name: str) -> BLSResult:
        """Запуск BLS поиска"""
        try:
            result = await self.bls_service.analyze(
                time, flux, flux_err, period_min, period_max, 7.0, target_name
            )
            logger.info(f"✅ BLS search completed: P={result.best_period:.3f}d, SNR={result.snr:.1f}")
            return result
        except Exception as e:
            logger.error(f"❌ BLS search failed: {e}")
            return None
    
    async def _run_gpi_search(self, time: np.ndarray, flux: np.ndarray, target_name: str) -> Dict:
        """Запуск GPI поиска"""
        try:
            if self.gpi_service:
    ...
    async def _run_tls_search(self, time: np.ndarray, flux: np.ndarray, 
                            period_min: float, period_max: float) -> Dict:
    ...
    async def _run_wavelet_analysis(self, time: np.ndarray, flux: np.ndarray) -> Dict:
        ...
        try:
            ...
            cwt_matrix = scipy_signal.cwt(flux - np.mean(flux), scipy_signal.ricker, widths)
    ...
    async def _run_fourier_analysis(self, time: np.ndarray, flux: np.ndarray, 
                                  period_min: float, period_max: float) -> Dict:
        ...
        try:
            ...
            if len(filtered_power) > 0:
                ...
                # SNR
                snr = best_power / np.std(filtered_power) if np.std(filtered_power) > 0 else 0
    ...
    async def _run_ml_ensemble_search(self, all_features: Dict[str, np.ndarray], 
                                    time: np.ndarray, flux: np.ndarray) -> Dict:
        """Запуск ML ансамбля"""
        try:
            ...
            for feature_type, features in all_features.items():
                ...
                feature_names.extend([f"{feature_type}_{i}" for i in range(len(features))])
            ...
            if len(stat_features) > 1:
                confidence = min(1.0, stat_features[1] / stat_features[0]) if stat_features[0] > 0 else 0.5
            else:
                confidence = 0.5
    ...
    async def _ultimate_ensemble_combination(self, method_results: Dict, all_features: Dict,
                                           time: np.ndarray, flux: np.ndarray, target_name: str) -> Dict:
        ...
        for method_name, result in method_results.items():
            ...
            if method_name == 'bls' and hasattr(result, 'best_period'):
                ...
                weights.append(method_weight * (1 + result.snr / 20))  # Бонус за высокий SNR
        ...
        consensus_depth = await self._estimate_transit_depth(time, flux, consensus_period)
        ...
        logger.info(f"🎯 CONSENSUS: P={consensus_period:.3f}d, Conf={consensus_confidence:.3f}")
        ...
        return {
            ...
            'planetary_system': await self._model_planetary_system(consensus_period, consensus_depth),
            'orbital_dynamics': await self._calculate_orbital_dynamics(consensus_period),
            'quality_flags': await self._assess_data_quality(time, flux, all_features)
        }
    
    async def _calculate_period_agreement(self, periods: List[float], weights: np.ndarray) -> float:
    ...
    async def _estimate_transit_depth(self, time: np.ndarray, flux: np.ndarray, period: float) -> float:
        """Оценка глубины транзита"""
        try:
            # Фазовая свертка
            phase = (time % period) / period
    ...
    async def _bootstrap_analysis(self, method_results: Dict, time: np.ndarray, 
                                flux: np.ndarray, n_bootstrap: int = 100) -> Dict:
        """Bootstrap анализ для оценки неопределенности"""
        try:
            bootstrap_periods = []
            ...
            for _ in range(n_bootstrap):
                bootstrap_sample = np.random.choice(periods, size=len(periods), replace=True)
                bootstrap_periods.append(np.mean(bootstrap_sample))
            
            # Статистика bootstrap
            bootstrap_periods = np.array(bootstrap_periods)
            uncertainty = np.std(bootstrap_periods) / np.mean(bootstrap_periods) if np.mean(bootstrap_periods) > 0 else 1.0
            
            # Доверительный интервал
            confidence_interval = np.percentile(bootstrap_periods, [2.5, 97.5])
            confidence = 1.0 - (confidence_interval[1] - confidence_interval[0]) / np.mean(bootstrap_periods)
    ...
    async def _identify_planet_candidates(self, method_results: Dict, consensus_period: float, 
                                        consensus_confidence: float) -> List[Dict]:
        ...
        try:
            ...
            if consensus_confidence > 0.7:
                candidates.append({
                    ...
                    'type': 'primary',
                    'methods_supporting': len([r for r in method_results.values() if r is not None])
                })
            ...
            for method_name, result in method_results.items():
                ...
                if hasattr(result, 'secondary_periods'):
                    for sec_period in result.secondary_periods[:2]:  # Максимум 2 дополнительных
    ...
    async def _estimate_false_positive_probability(self, method_results: Dict, all_features: Dict,
                                                 consensus_period: float, consensus_snr: float) -> float:
        ...
        try:
            ...
            method_agreement = len([r for r in method_results.values() if r is not None]) / len(method_results)
            ...
            if len(stat_features) > 5:
                ...
                variance_factor = min(2.0, stat_features[1] * 10)  # Нормализованная дисперсия
    ...
    async def _calculate_detection_significance(self, snr: float, agreement: float, 
                                             num_methods: int) -> float:
    ...
    async def _cross_validate_results(self, method_results: Dict, all_features: Dict) -> float:
    ...
    async def _estimate_stellar_parameters(self, time: np.ndarray, flux: np.ndarray) -> Dict:
        ...
        try:
            ...
            estimated_magnitude = 12.0 + np.log10(flux_std * 1000)  # Эмпирическая формула
    ...
    async def _assess_data_quality(self, time: np.ndarray, flux: np.ndarray, 
