...
from services.ensemble_search_service import UltimateEnsembleSearchEngine, EnsembleSearchResult
...
class UnifiedSearchService:
    ...
    async def search(self, request: UnifiedSearchRequest) -> UnifiedSearchResult:
        ...
        try:
            ...
            await self._update_performance_stats(request.search_mode, processing_time, result)
            
            # Генерируем рекомендации
            result.recommendations = await self._generate_recommendations(result, request)
    ...
    async def _run_bls_search(self, request: UnifiedSearchRequest) -> UnifiedSearchResult:
        """Запуск BLS поиска"""
    ...
    async def _run_ensemble_search(self, request: UnifiedSearchRequest) -> UnifiedSearchResult:
        ...
        logger.info(f"✅ ENSEMBLE Search completed: P={ensemble_result.consensus_period:.3f}d")
        return unified_result
    
    async def _run_hybrid_search(self, request: UnifiedSearchRequest) -> UnifiedSearchResult:
        ...
        if request.use_parallel:
            bls_task = asyncio.create_task(self._run_bls_search(
            ...
            ensemble_task = asyncio.create_task(self._run_ensemble_search(
                UnifiedSearchRequest(
                    target_name=request.target_name,
                    time=request.time,
                    flux=request.flux,
    ...
    async def _combine_hybrid_results(self, bls_result: UnifiedSearchResult, 
                                    ensemble_result: UnifiedSearchResult,
                                    request: UnifiedSearchRequest) -> UnifiedSearchResult:
        ...
        mode_comparison['choice_reason'] = self._explain_method_choice(mode_comparison, chosen_mode)
        
        return hybrid_result
    
    async def _compare_results(self, bls_result: UnifiedSearchResult, 
                             ensemble_result: UnifiedSearchResult) -> Dict:
        ...
        period_agreement = 1.0 - (period_diff / max(bls_result.best_period, ensemble_result.best_period))
        
        snr_ratio = ensemble_result.snr / bls_result.snr if bls_result.snr > 0 else 1.0
        confidence_ratio = ensemble_result.confidence / bls_result.confidence if bls_result.confidence > 0 else 1.0
        ...
        return {
            ...
            'ensemble_metrics': {
                ...
                'method_agreement': float(ensemble_result.ensemble_result.method_agreement) if ensemble_result.ensemble_result else 0.0
            }
        }
    ...
    async def _calculate_ensemble_quality_score(self, ensemble_result: EnsembleSearchResult) -> float:
    ...
    async def _generate_recommendations(self, result: UnifiedSearchResult, 
        ...
        if result.snr < 7:
            recommendations.append("Низкий SNR - рекомендуется дополнительная валидация")
        elif result.snr > 15:
        ...
        if request.search_mode == SearchMode.BLS and result.confidence < 0.7:
            ...
            if result.ensemble_result.method_agreement < 0.6:
                recommendations.append("Низкое согласие между методами - требуется осторожность")
        ...
        if result.best_period < 1.0:
            ...
            recommendations.append("Длинный период - требуется больше данных для подтверждения")
        ...
        if result.depth < 0.001:
            ...
            recommendations.append("Глубокий транзит - проверьте на затмевающие двойные")
        
        return recommendations
    
    async def _update_performance_stats(self, mode: SearchMode, processing_time: float, 
