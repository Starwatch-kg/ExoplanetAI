...
class BLSService:
    ...
    def _calculate_secondary_periods(self, periods: np.ndarray, powers: np.ndarray, 
                                   primary_period: float) -> List[float]:
        ...
        return [secondary_periods[i] for i in sorted_indices[:5] if i < len(secondary_periods)]
    
    def _calculate_period_aliases(self, primary_period: float, time_span: float) -> List[float]:
    ...
    def _calculate_physical_parameters(self, period: float, depth: float, duration: float,
                                     stellar_mass: float = 1.0, stellar_radius: float = 1.0) -> Dict[str, float]:
    ...
    def _calculate_quality_metrics(self, time: np.ndarray, flux: np.ndarray, 
                                 period: float, depth: float) -> Dict[str, float]:
        ...
        if np.sum(in_transit) > 10:
            ...
            if np.sum(odd_mask) > 2 and np.sum(~odd_mask) > 2:
                ...
                odd_even_mismatch = abs(odd_depth - even_depth) / depth if depth > 0 else 0
            else:
                odd_even_mismatch = 0
        else:
            odd_even_mismatch = 0
        ...
        return {
            ...
            'completeness': float(len(time) / ((np.max(time) - np.min(time)) / np.median(np.diff(time))))
        }
    ...
    async def analyze(
        ...
        logger.info(f"Data: {len(time)} points, period range: {period_min}-{period_max} days")
        ...
        try:
            ...
            time_clean, flux_clean = await self._advanced_data_cleaning(time, flux, flux_err)
            ...
            secondary_periods = self._calculate_secondary_periods(periods, powers, best_period)
            period_aliases = self._calculate_period_aliases(best_period, np.max(time_clean) - np.min(time_clean))
            
            # Physical parameters
            physical_params = self._calculate_physical_parameters(best_period, depth, duration)
            
            # Quality metrics
            quality_metrics = self._calculate_quality_metrics(time_clean, flux_clean, best_period, depth)
            ...
            is_significant = (
                ...
                depth > 0.0005 and 
                false_alarm_prob < 0.01 and
                quality_metrics['odd_even_mismatch'] < 0.5
            )
            ...
            self.performance_stats['avg_processing_time'] = (
                self.performance_stats['avg_processing_time'] * (self.performance_stats['total_searches'] - 1) + 
                processing_time
            ) / self.performance_stats['total_searches']
            
            if not any(np.isnan([snr, depth, significance])):
                self.performance_stats['success_rate'] = (
                    self.performance_stats['success_rate'] * (self.performance_stats['total_searches'] - 1) + 1
                ) / self.performance_stats['total_searches']
    ...
    async def _advanced_data_cleaning(self, time: np.ndarray, flux: np.ndarray, 
    ...
    async def _create_intelligent_period_grid(self, time: np.ndarray, 
                                            period_min: float, period_max: float) -> np.ndarray:
    ...
    async def _calculate_advanced_snr(self, time: np.ndarray, flux: np.ndarray,
                                    period: float, depth: float, duration: float) -> float:
    ...
    async def _calculate_false_alarm_probability(self, powers: np.ndarray, 
                                               best_power: float, n_periods: int) -> float:
    ...
    async def _calculate_statistical_significance(self, snr: float, fap: float, 
