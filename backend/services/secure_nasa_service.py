...
from core.validators import validate_target_name, validate_catalog, SecurityError
...
class SecureNASAService:
    ...
    async def initialize(self):
        ...
        if not ASTRONOMY_LIBS_AVAILABLE:
            logger.warning("⚠️ Astronomy libraries not available. Install: pip install lightkurve astroquery")
        else:
    ...
    async def _secure_http_request(self, url: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """Make secure HTTP request with validation"""
        if not self.session:
            await self.initialize()
        
        # Validate URL
        if not url.startswith(('https://exoplanetarchive.ipac.caltech.edu', 'https://mast.stsci.edu')):
    ...
    async def search_exoplanet_archive(self, target_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                logger.info(f"✅ Found {len(result)} entries for {target_name} in NASA archive")
                return data
            else:
    ...
    def _process_archive_result(self, result, target_name: str) -> Dict[str, Any]:
        ...
        if len(result) > 0:
            ...
            data["host_star"] = {
                ...
                "magnitude": float(first_row['sy_vmag']) if first_row['sy_vmag'] else None,
                "temperature": float(first_row['st_teff']) if first_row['st_teff'] else None,
            ...
            for row in result:
                planet_data = {
                    ...
                    "discovery_facility": str(row['disc_facility']) if row['disc_facility'] else None
                }
                data["planets"].append(planet_data)
        
        return data
    
    async def get_tess_lightcurve(self, target_name: str, sector: Optional[int] = None) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        """Get TESS light curve data for target"""
        # Input validation
        target_name = validate_target_name(target_name)
        
        cache_key = f"tess_lc_{hashlib.md5(f'{target_name}_{sector}'.encode()).hexdigest()}"
        if self._is_cached(cache_key):
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"✅ Downloaded TESS data for {target_name}: {len(time_data)} points")
                return result
            else:
    ...
    def _sync_download_tess(self, target_name: str, sector: Optional[int] = None) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        """Synchronous TESS data download"""
        try:
            # Search for TESS observations
            search_result = lk.search_lightcurve(target_name, mission='TESS', sector=sector)
            ...
            # Data quality checks
            if len(lc.flux) < 100:
                logger.warning(f"Insufficient TESS data points for {target_name}: {len(lc.flux)}")
                return None
            ...
            flux_err_data = lc.flux_err.value if lc.flux_err is not None else np.ones_like(flux_data) * 0.001
    ...
    async def get_kepler_lightcurve(self, target_name: str, quarter: Optional[int] = None) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        """Get Kepler light curve data for target"""
        # Input validation
        target_name = validate_target_name(target_name)
        
        cache_key = f"kepler_lc_{hashlib.md5(f'{target_name}_{quarter}'.encode()).hexdigest()}"
        if self._is_cached(cache_key):
        ...
        try:
            ...
            if result:
                self._cache_data(cache_key, result)
                time_data, flux_data, flux_err_data = result
                logger.info(f"✅ Downloaded Kepler data for {target_name}: {len(time_data)} points")
                return result
            else:
    ...
    def _sync_download_kepler(self, target_name: str, quarter: Optional[int] = None) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        """Synchronous Kepler data download"""
        try:
            # Search for Kepler observations
            search_result = lk.search_lightcurve(target_name, mission='Kepler', quarter=quarter)
            ...
            # Data quality checks
            if len(lc.flux) < 100:
                logger.warning(f"Insufficient Kepler data points for {target_name}: {len(lc.flux)}")
                return None
