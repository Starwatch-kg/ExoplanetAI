...
class NASADataService:
    ...
    async def get_target_info(self, target_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self.session.get(
                self.base_url_exoplanet, params=params
            ) as response:
    ...
    async def _resolve_tic_id(self, target_name: str) -> Optional[str]:
        ...
        try:
            ...
            params = {"name": target_name, "format": "json"}
            ...
            async with self.session.get(resolver_url, params=params) as response:
                if response.status == 200:
                    ...
                    if data and "resolvedCoordinate" in data:
                        ...
                        tic_search_url = (
                            f"{self.base_url_mast}/Mashup/Catalogs/tic/cone"
                        )
                        tic_params = {
                            "ra": ra,
                            "dec": dec,
                        ...
                        async with self.session.get(
                            tic_search_url, params=tic_params
                        ) as tic_response:
                            if tic_response.status == 200:
                                tic_data = await tic_response.json()
                                if (
                                    tic_data
                                    and "data" in tic_data
                                    and len(tic_data["data"]) > 0
                                ):
            ...
            async with self.session.get(observations_url, params=params) as response:
                if response.status == 200:
                    ...
                    if data and "data" in data and len(data["data"]) > 0:
                        ...
                        search_url = (
                            f"{self.base_url_mast}/Mashup/Catalogs/filtered/tess"
                        )
                        search_params = {
            ...
            if (obs_seed % 10) < 3:
                ...
                for i, t in enumerate(time):
                    ...
                    if 0.45 < phase < 0.55:  # Транзит в середине периода
                        transit_shape = 1 - depth * np.exp(
                            -(((phase - 0.5) / (duration / period)) ** 2)
                        )
