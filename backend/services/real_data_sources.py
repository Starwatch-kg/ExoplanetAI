...
class RealDataSourceManager:
    ...
    async def search_nasa_exoplanet_archive(self, target_name: str) -> Optional[Dict[str, Any]]:
            ...
            if len(result) > 0:
                data = {
                    ...
                    "host_star": {
                        ...
                        "magnitude": float(result[0]['sy_vmag']) if result[0]['sy_vmag'] else None,
                        "temperature": float(result[0]['st_teff']) if result[0]['st_teff'] else None,
                        "radius": float(result[0]['st_rad']) if result[0]['st_rad'] else None,
                        "mass": float(result[0]['st_mass']) if result[0]['st_mass'] else None
                    },
                    "planets": []
                }
    ...
    async def get_tess_lightcurve(self, target_name: str, sector: Optional[int] = None) -> Optional[LightCurveData]:
        ...
        try:
            ...
            lc_data = LightCurveData(
                ...
                quality=lc.quality.value if hasattr(lc, 'quality') else np.zeros_like(lc.flux.value),
            ...
            logger.info(f"✅ Downloaded TESS data for {target_name}: {len(lc.flux)} points")
            return lc_data
    ...
    async def get_kepler_lightcurve(self, target_name: str, quarter: Optional[int] = None) -> Optional[LightCurveData]:
        ...
        try:
            ...
            self._cache_data(cache_key, lc_data)
            logger.info(f"✅ Downloaded Kepler data for {target_name}: {len(lc.flux)} points")
            return lc_data
    ...
    async def get_multi_mission_data(self, target_name: str) -> Dict[str, Optional[LightCurveData]]:
        ...
        try:
            if ASTRONOMY_LIBS_AVAILABLE:
                ...
                if len(search_result) > 0:
                    k2_data = await self.get_kepler_lightcurve(target_name)  # Similar processing
                    results['K2'] = k2_data
        except Exception as e:
...
async def get_real_lightcurve_data(target_name: str, mission: str = "TESS") -> Optional[LightCurveData]:
    """
